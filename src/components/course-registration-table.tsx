...
import type { CourseRegistration } from '@/types/student';
...
export function CourseRegistrationTable({ registrations }: CourseRegistrationTableProps) {
  ...
  // Lấy tất cả subjects từ registrations
  const allSubjects = useMemo(() => {
    const subjectSet = new Set<string>();
    registrations.forEach(reg => {
      reg.registeredSubjects.forEach(subject => {
  ...
  const columns = useMemo<ColumnDef<CourseRegistration>[]>(
    () => {
      ...
      const subjectColumns: ColumnDef<CourseRegistration>[] = allSubjects.map((subject) => ({
        id: `subject-${subject}`,
        header: subject,
        cell: (info) => {
          const registration = info.row.original;
          const isRegistered = registration.registeredSubjects.includes(subject);
          ...
          return (
            <div className="flex flex-col items-center py-0.5 px-1 rounded bg-blue-100">
